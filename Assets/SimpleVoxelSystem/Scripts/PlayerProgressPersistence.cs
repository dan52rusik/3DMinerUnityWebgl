...
namespace SimpleVoxelSystem
{
    ...
    public class PlayerProgressPersistence : MonoBehaviour
    {
        ...
        private void ApplyLoadedState(string json)
        {
            ...
            // FIX: After restoring island + mines + blocks, defer player spawn by one
            // FixedUpdate so that the MeshColliders generated by RebuildMesh() have time
            // to bake. Without this, Physics.Raycast in TryGetSurfaceYAtOnActiveIsland
            // can miss, causing the player to spawn at a default position and fall through.
            if (!wellGenerator.IsInLobbyMode && wellGenerator.PrivateIsland != null)
            {
                StartCoroutine(DeferredIslandSpawn());
            }
        ...
        private System.Collections.IEnumerator DeferredIslandSpawn()
        {
            // Wait one physics step for MeshColliders to be baked
            yield return new WaitForFixedUpdate();
            // Extra safety: wait one more frame for Physics.SyncTransforms
            yield return null;
