...
namespace YG
{
    public partial class PlatformYG2 : IPlatformsYG2
    {
        ...
        {
            string domain = ResolveYandexDomain();
        ...
        private static string ResolveYandexDomain()
        {
            string absoluteUrl = Application.absoluteURL;
            if (string.IsNullOrWhiteSpace(absoluteUrl))
                return "ru";

            if (!System.Uri.TryCreate(absoluteUrl, System.UriKind.Absolute, out System.Uri uri))
                return "ru";

            string host = uri.Host.ToLowerInvariant();
            int domainIndex = host.IndexOf("yandex.");
            if (domainIndex < 0)
                return "ru";
            ...
            return string.IsNullOrWhiteSpace(suffix) ? "ru" : suffix;
        }
    }
}
#endif
